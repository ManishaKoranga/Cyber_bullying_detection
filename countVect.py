...
vectoriser = CountVectorizer(min_df=1, ngram_range=(1,1), stop_words='english')
vectoriser.fit_transform(X_train)
pickle.dump(vectoriser, open('countVectorizer', 'wb'))
...
print('Accuracy with CountVectorizer + SVM:'+ str(accuracy))

# dumping the model
pickle.dump(svm_model_linear, open('modelCV.bin', 'wb'))
...
def custom_input_prediction(text):
    ...
    text = [text[0],]
    vectoriser = pickle.load(open("countVectorizer", "rb"))
    text = vectoriser.transform(text)
    model = pickle.load(open("modelCV.bin", "rb"))
